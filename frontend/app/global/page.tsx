...
const card: React.CSSProperties = {
  background: "rgba(0,14,5,0.9)", border: "1px solid rgba(0,255,80,0.15)",
  borderRadius: 14, padding: "20px 24px",
};
const inp: React.CSSProperties = {
  background: "rgba(0,12,4,0.95)", border: "1px solid rgba(0,255,80,0.3)",
  borderRadius: 8, color: "#e8f5eb", padding: "10px 14px",
  fontSize: 15, outline: "none", width: "100%", boxSizing: "border-box",
};

const RATING_COLOR: Record<string, string> = {
  excellent: "#00ff80", good: "#66ffaa", ok: "#ffcc44", marginal: "#ff9944", loss: "#ff4444",
};
...
function PlatformRow({ p, rank, hasBuyPrice }: { p: PlatformData; rank: number; hasBuyPrice: boolean }) {
  ...
  const profitColor = p.profit
    ? (RATING_COLOR[p.profit.rating ?? (p.profit.is_profitable ? "ok" : "loss")] ?? "#e8f5eb")
    : "#4a8a5a";
    ...
    <>
      ...
      <tr
        ...
        style={{
          ...
          borderBottom: "1px solid rgba(0,255,80,0.07)",
          background: open ? "rgba(0,255,80,0.03)" : "transparent",
          transition: "background 0.12s",
        }}
      >
        {/* 順位 */}
        <td style={{ padding: "14px 10px", textAlign: "center", fontWeight: 700, color: rank <= 3 && hasBuyPrice ? "#00ff80" : "#3a6a4a", width: 40 }}>
        ...
        <td style={{ padding: "14px 10px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div>
              <div style={{ fontSize: 14, fontWeight: 700, color: "#e8f5eb" }}>{p.name}</div>
              <div style={{ fontSize: 11, color: "#4a8a5a" }}>{p.area} · {(p.fee_rate * 100).toFixed(1)}%手数料</div>
            </div>
          </div>
        </td>
        ...
        <td style={{ padding: "14px 10px", textAlign: "right" }}>
          ...
          {p.currency !== "JPY" && (
            <div style={{ fontSize: 11, color: "#4a8a5a" }}>
        ...
        {/* 平均価格 */}
        <td style={{ padding: "14px 10px", textAlign: "right" }}>
          <div style={{ fontSize: 16, fontWeight: 700, color: "#e8f5eb" }}>
            {fmtPrice(p.avg_price_local, p.currency)}
          </div>
          {p.currency !== "JPY" && (
            <div style={{ fontSize: 11, color: "#4a8a5a" }}>≈ ¥{p.avg_price_jpy.toLocaleString()}</div>
          )}
          <div style={{ fontSize: 11, color: "#4a8a5a", marginTop: 2 }}>{p.item_count}件</div>
        </td>
      ...
      {open && (
        <tr>
          <td colSpan={hasBuyPrice ? 7 : 6} style={{ padding: "0 10px 16px", background: "rgba(0,255,80,0.02)" }}>
              ...
              {p.items.map((item, idx) => (
                <a
                  ...
                  style={{
                    ...
                    background: "rgba(0,12,4,0.7)", borderRadius: 8, padding: "8px 12px",
                    textDecoration: "none", border: "1px solid rgba(0,255,80,0.08)",
                  }}
                >
                  {item.image && (
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, color: "#e8f5eb", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {item.name}
                    </div>
                    {item.condition && <div style={{ fontSize: 11, color: "#4a8a5a" }}>{item.condition}</div>}
                  </div>
                  <div style={{ textAlign: "right", flexShrink: 0 }}>
                    ...
                    {p.currency !== "JPY" && (
                      <div style={{ fontSize: 11, color: "#4a8a5a" }}>≈ ¥{Math.round(item.price_jpy).toLocaleString()}</div>
                    )}
                  </div>
    ...
    <div style={{ maxWidth: 1100, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
          <Globe size={24} color="#00ff80" />
          <h1 style={{ fontSize: 24, fontWeight: 900, color: "#e8f5eb", margin: 0 }}>
            全プラットフォーム相場比較
          </h1>
        </div>
        <p style={{ fontSize: 13, color: "#4a8a5a", margin: 0 }}>
      ...
      <div style={{ ...card, marginBottom: 24 }}>
        ...
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 12, color: "#4a8a5a", fontWeight: 600, marginBottom: 8, display: "flex", alignItems: "center", gap: 6 }}>
            <Camera size={13} color="#4a8a5a" />
            写真から商品を識別（AI自動認識）
          ...
          {imagePreview ? (
            <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
              <div style={{ position: "relative", flexShrink: 0 }}>
                <img src={imagePreview} alt="アップロード画像" style={{ width: 80, height: 80, objectFit: "cover", borderRadius: 10, border: "1px solid rgba(0,255,80,0.25)" }} />
              ...
              <div style={{ flex: 1 }}>
                {imgLoading ? (
                  ...
                  <div style={{ fontSize: 13, color: "#00ff80" }}>
            ...
            <div
              ...
              style={{
                border: "2px dashed rgba(0,255,80,0.2)", borderRadius: 10,
                padding: "20px 16px", textAlign: "center", cursor: "pointer",
                background: "rgba(0,255,80,0.02)", transition: "border-color 0.15s",
        ...
        <div style={{ borderTop: "1px solid rgba(0,255,80,0.08)", marginBottom: 16 }} />

        <div style={{ display: "flex", gap: 12, alignItems: "flex-end", flexWrap: "wrap" }}>
          {/* キーワード */}
          <div style={{ flex: 2, minWidth: 220 }}>
            <label style={{ fontSize: 12, color: "#4a8a5a", display: "block", marginBottom: 5, fontWeight: 600 }}>
              商品名・キーワード
            </label>
            <div style={{ position: "relative" }}>
              <Search size={15} style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "#4a8a5a" }} />
              <input
                ref={kwRef}
          ...
          <button
            ...
            style={{
              background: loading ? "rgba(0,255,80,0.05)" : "linear-gradient(135deg,#004d1f,#006629)",
              border: "1px solid rgba(0,255,80,0.4)", borderRadius: 10, color: "#00ff80",
      ...
      {loading && (
        <div style={{ ...card, textAlign: "center", padding: "48px 24px" }}>
          <RefreshCw size={32} color="#00ff80" style={{ animation: "spin 1s linear infinite", margin: "0 auto 16px" }} />
          <div style={{ color: "#e8f5eb", fontWeight: 700, fontSize: 16, marginBottom: 6 }}>全プラットフォームを検索中...</div>
          <div style={{ color: "#4a8a5a", fontSize: 13 }}>
        ...
        <>
          ...
          <div style={{ display: "flex", gap: 14, marginBottom: 20, flexWrap: "wrap" }}>
            <div style={{ ...card, flex: 1, minWidth: 180, padding: "14px 20px" }}>
              <div style={{ fontSize: 11, color: "#4a8a5a", marginBottom: 4 }}>検索キーワード</div>
              <div style={{ fontSize: 18, fontWeight: 800, color: "#e8f5eb" }}>「{result.keyword}」</div>
            </div>
            <div style={{ ...card, flex: 1, minWidth: 160, padding: "14px 20px" }}>
              <div style={{ fontSize: 11, color: "#4a8a5a", marginBottom: 4 }}>データ取得プラットフォーム</div>
              <div style={{ fontSize: 22, fontWeight: 800, color: "#00ff80" }}>{result.platforms.length} サイト</div>
            </div>
            {bestPlatform && (
              <div style={{ ...card, flex: 2, minWidth: 260, padding: "14px 20px", borderColor: "rgba(0,255,80,0.4)" }}>
                <div style={{ fontSize: 11, color: "#4a8a5a", marginBottom: 4 }}>
                  {hasBuyPrice ? "🥇 最も利益が出るプラットフォーム" : "💰 最も高く売れるプラットフォーム"}
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ fontSize: 24 }}>{bestPlatform.flag}</span>
                  <div>
                    <div style={{ fontSize: 16, fontWeight: 800, color: "#00ff80" }}>{bestPlatform.name}</div>
                    <div style={{ fontSize: 13, color: "#e8f5eb" }}>
                      ...
                      {hasBuyPrice && bestPlatform.profit && (
                        <span style={{ color: "#00ff80", fontWeight: 700, marginLeft: 10 }}>
          ...
          <div style={card}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
              ...
              <span style={{ fontWeight: 700, color: "#e8f5eb", fontSize: 15 }}>
                {hasBuyPrice ? "利益ランキング" : "販売相場一覧"}
              </span>
              <span style={{ fontSize: 12, color: "#4a8a5a" }}>
                {hasBuyPrice
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
                <thead>
                  <tr style={{ borderBottom: "1px solid rgba(0,255,80,0.2)" }}>
                    <th style={{ padding: "8px 10px", color: "#4a8a5a", fontWeight: 600, textAlign: "center", width: 40 }}>#</th>
                    <th style={{ padding: "8px 10px", color: "#4a8a5a", fontWeight: 600, textAlign: "left" }}>プラットフォーム</th>
                    <th style={{ padding: "8px 10px", color: "#4a8a5a", fontWeight: 600, textAlign: "right" }}>最安値</th>
                    <th style={{ padding: "8px 10px", color: "#4a8a5a", fontWeight: 600, textAlign: "right" }}>平均価格</th>
                    <th style={{ padding: "8px 10px", color: "#4a8a5a", fontWeight: 600, textAlign: "right" }}>最高値</th>
                    {hasBuyPrice && (
                      <th style={{ padding: "8px 10px", color: "#4a8a5a", fontWeight: 600, textAlign: "right" }}>利益（推定）</th>
                    )}
                    <th style={{ width: 32 }} />
                  </tr>
                </thead>
      ...
      {!result && !loading && !error && (
        <div style={{ ...card, textAlign: "center", padding: "56px 24px", borderStyle: "dashed" }}>
          ...
          <div style={{ fontSize: 16, fontWeight: 700, color: "#4a8a5a", marginBottom: 8 }}>
            商品名を入力して検索してください
          </div>
            ...
            {["Nintendo Switch", "ポケモンカード", "AirPods Pro", "LEGO"].map(kw => (
              <button
                ...
                style={{
                  ...
                  borderRadius: 20, color: "#4a8a5a", padding: "6px 16px", fontSize: 13, cursor: "pointer",
                }}
              >
                {kw}
              </button>
