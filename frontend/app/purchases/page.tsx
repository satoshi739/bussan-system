...
const inp: React.CSSProperties = { background: "rgba(0,12,4,0.95)", border: "1px solid rgba(0,255,80,0.3)", borderRadius: 8, color: "#e8f5eb", padding: "9px 12px", fontSize: 14, width: "100%", outline: "none", boxSizing: "border-box" };
...
const STATUS = {
  ...
  sold:      { label: "売却済み",  color: "#00ff80", bg: "rgba(0,255,128,0.12)" },
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 900, color: "#e8f5eb", margin: 0 }}>仕入れ管理</h1>
          <div style={{ fontSize: 12, color: "#4a8a5a", marginTop: 3 }}>仕入れた商品の一覧・ステータス管理</div>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <a href="http://localhost:8000/api/purchases/export/csv" download style={{ display: "flex", alignItems: "center", gap: 6, background: "rgba(0,40,15,0.8)", border: "1px solid rgba(0,255,80,0.25)", borderRadius: 10, color: "#8ab89a", padding: "10px 14px", fontWeight: 700, fontSize: 13, cursor: "pointer", textDecoration: "none" }}>
          ...
          <button
            ...
            style={{ display: "flex", alignItems: "center", gap: 6, background: isFree && allItemsCount >= FREE_PLAN_LIMIT ? "rgba(0,20,8,0.7)" : "linear-gradient(135deg,#004d1f,#006629)", border: `1px solid ${isFree && allItemsCount >= FREE_PLAN_LIMIT ? "rgba(255,80,50,0.3)" : "rgba(0,255,80,0.4)"}`, borderRadius: 10, color: isFree && allItemsCount >= FREE_PLAN_LIMIT ? "#ff9977" : "#00ff80", padding: "10px 18px", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, marginBottom: 20 }}>
        {[
          ...
          { key: "sold",      label: "売却済み", icon: <CheckCircle size={18} />, color: "#00ff80", extra: `${byStatus("sold").length}件` },
          ...
          <button
            ...
            style={{ background: filter === key ? `${color}18` : "rgba(0,14,5,0.9)", border: `1px solid ${filter === key ? color + "55" : "rgba(0,255,80,0.1)"}`, borderRadius: 12, padding: "14px 16px", cursor: key ? "pointer" : "default", textAlign: "left", transition: "all 0.15s" }}
          >
            ...
            <div style={{ fontSize: 22, fontWeight: 900, color: "#e8f5eb", fontFamily: "monospace" }}>{key ? byStatus(key).length : ""}<span style={{ fontSize: 11, color: "#4a8a5a", marginLeft: 2 }}>{key ? "件" : ""}</span></div>
      ...
      {/* ── 追加フォーム ── */}
      {showForm && (
        <div style={{ background: "rgba(0,14,5,0.9)", border: "1px solid rgba(0,255,80,0.3)", borderRadius: 14, padding: "20px 24px", marginBottom: 16 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: "#b8dcc4", marginBottom: 16 }}>新規仕入れ登録</div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={handleSubmit} disabled={loading} style={{ background: "linear-gradient(135deg,#004d1f,#006629)", border: "1px solid rgba(0,255,80,0.4)", borderRadius: 8, color: "#00ff80", padding: "10px 28px", fontWeight: 700, cursor: "pointer", fontSize: 14 }}>{loading ? "保存中..." : "保存する"}</button>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 14, alignItems: "center" }}>
        <div style={{ position: "relative", flex: 1 }}>
          <Search size={14} style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", color: "#4a8a5a" }} />
        ...
        <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
          {(["purchased", "listed", "sold", ""] as const).map(s => (
            <button key={s} onClick={() => setFilter(s)} style={{ padding: "7px 14px", borderRadius: 20, border: `1px solid ${filter === s ? "rgba(0,255,80,0.4)" : "rgba(0,255,80,0.12)"}`, background: filter === s ? "rgba(0,255,80,0.12)" : "transparent", color: filter === s ? "#00ff80" : "#8ab89a", fontWeight: 600, fontSize: 12, cursor: "pointer", whiteSpace: "nowrap" }}>
      ...
      {quickSell && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={e => e.target === e.currentTarget && setQuickSell(null)}>
          <div style={{ background: "#060f08", border: "1px solid rgba(0,255,80,0.3)", borderRadius: 16, padding: 28, width: 560, maxHeight: "90vh", overflowY: "auto", position: "relative" }}>
            <button onClick={() => setQuickSell(null)} style={{ position: "absolute", top: 14, right: 14, background: "transparent", border: "none", color: "#8ab89a", cursor: "pointer" }}><X size={18} /></button>
            ...
            <div style={{ background: "rgba(0,12,4,0.8)", borderRadius: 8, padding: "10px 14px", marginBottom: 16, border: "1px solid rgba(0,255,80,0.1)" }}>
              <div style={{ fontSize: 14, fontWeight: 600, color: "#e8f5eb" }}>{quickSell.product_name}</div>
              <div style={{ fontSize: 12, color: "#4a8a5a", marginTop: 2 }}>仕入れ: {quickSell.platform} · コスト ¥{(quickSell.purchase_price + quickSell.purchase_shipping).toLocaleString()}</div>
            </div>
            ...
            {sortedComparison.length > 0 ? (
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 12, color: "#4a8a5a", fontWeight: 700, marginBottom: 8 }}>どこで売ると一番儲かるか</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                  {sortedComparison.map(([name, data], i) => {
                    ...
                    return (
                      <button key={name} onClick={() => setSellPlatform(name)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 14px", borderRadius: 9, border: isSelected ? "1px solid rgba(0,255,80,0.5)" : isBest ? "1px solid rgba(0,255,80,0.2)" : "1px solid rgba(0,255,80,0.07)", background: isSelected ? "rgba(0,255,80,0.08)" : isBest ? "rgba(0,255,80,0.04)" : "rgba(0,12,4,0.5)", cursor: "pointer", width: "100%" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          {isSelected && <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#00ff80" }} />}
                          <span style={{ fontSize: 13, color: "#e8f5eb" }}>{data.emoji} {name}</span>
                          {isBest && <span style={{ fontSize: 10, background: "rgba(0,255,80,0.2)", border: "1px solid rgba(0,255,80,0.3)", borderRadius: 10, padding: "1px 6px", color: "#00ff80" }}>最高</span>}
                        </div>
                        <div style={{ textAlign: "right" }}>
                          <span style={{ fontSize: 15, fontWeight: 700, color: profit >= 0 ? "#00ff80" : "#ff6666", fontFamily: "monospace" }}>
              ...
              <div style={{ textAlign: "center", fontSize: 12, color: "#4a8a5a", marginBottom: 14, padding: "16px 0" }}>
            ...
            {/* 選択中のプラットフォーム表示 */}
            {sellPrice && (
              <div style={{ marginBottom: 14, padding: "8px 12px", background: "rgba(0,255,80,0.06)", borderRadius: 8, border: "1px solid rgba(0,255,80,0.2)", fontSize: 12, color: "#4a8a5a" }}>
                選択中: <span style={{ color: "#00ff80", fontWeight: 700 }}>{sellPlatform}</span>
              </div>
            )}

            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={handleQuickSell} disabled={loading || !sellPrice} style={{ flex: 1, background: "linear-gradient(135deg,#004d1f,#006629)", border: "1px solid rgba(0,255,80,0.4)", borderRadius: 8, color: "#00ff80", padding: "12px", fontWeight: 700, cursor: "pointer", fontSize: 14, opacity: (!sellPrice || loading) ? 0.5 : 1 }}>
                {loading ? "記録中..." : "売却を記録する"}
              </button>
              <button onClick={() => setQuickSell(null)} style={{ background: "transparent", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: "#8ab89a", padding: "12px 16px", cursor: "pointer" }}>キャンセル</button>
            </div>
          </div>
        </div>
      )}
      ...
      {filtered.length === 0 ? (
        <div style={{ background: "rgba(0,14,5,0.9)", border: "1px solid rgba(0,255,80,0.1)", borderRadius: 14, textAlign: "center", padding: 60 }}>
          <Package size={36} color="rgba(0,255,80,0.2)" style={{ margin: "0 auto 12px", display: "block" }} />
          <div style={{ color: "#4a8a5a", fontSize: 14 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {filtered.map(item => {
            ...
            return (
              <div key={item.id} style={{ background: "rgba(0,14,5,0.9)", border: "1px solid rgba(0,255,80,0.1)", borderRadius: 12, padding: "12px 16px", display: "flex", alignItems: "center", gap: 12, transition: "border-color 0.15s" }}>
                ...
                <div style={{ textAlign: "right", flexShrink: 0 }}>
                  <div style={{ fontFamily: "monospace", fontWeight: 700, color: "#e8f5eb", fontSize: 15 }}>¥{cost.toLocaleString()}</div>
                  {item.purchase_shipping > 0 && <div style={{ fontSize: 10, color: "#4a8a5a" }}>+送料¥{item.purchase_shipping.toLocaleString()}</div>}
                </div>

                {/* アクション */}
                <div style={{ display: "flex", gap: 5, flexShrink: 0 }}>
                  {item.status !== "sold" && item.status !== "cancelled" && (
                    <button onClick={() => openSell(item)} style={{ background: "rgba(0,80,30,0.7)", border: "1px solid rgba(0,255,80,0.3)", borderRadius: 8, color: "#4ddc80", cursor: "pointer", padding: "6px 12px", fontSize: 12, fontWeight: 700, display: "flex", alignItems: "center", gap: 4 }}>
                      <DollarSign size={12} /> 売却
                    </button>
                  )}
                  {item.purchase_url && (
                    <a href={item.purchase_url} target="_blank" rel="noreferrer" style={{ background: "transparent", border: "1px solid rgba(0,255,80,0.15)", borderRadius: 8, color: "#4a8a5a", padding: "6px 10px", display: "flex", alignItems: "center" }}>
